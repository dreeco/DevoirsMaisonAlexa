...
namespace DevoirsAlexa.Domain.Helpers;
...
internal class MathHelper
{
    ...
    {
        return new ExerciceRule($"Sub higher than {sum}", (key) =>
        {
    ...
    {
        return new ExerciceRule($"Sum lower than {sum}", (key) => GetNumbersInQuestion(key, '+').Sum() <= sum);
    }
    public static ExerciceRule GetRuleForNoComplicatedNumberAbove(int max)
    {
        return new ExerciceRule($"No complicated number above {max}", (key) => GetNumbersInQuestion(key).All(n => n % 5 == 0 || n <= max));
    }

    public static ExerciceRule GetRuleForNoNumberUnder(int min)
    {
        return new ExerciceRule($"No number under {min}", (key) => GetNumbersInQuestion(key).All(n => n >= min));
    }

    public static ExerciceRule GetRuleForNoNumberOver(int max)
    {
        return new ExerciceRule($"No number over {max}", (key) => GetNumbersInQuestion(key).All(n => n <= max));
    }

    public static ExerciceRule GetRuleForDifferentNumbers()
    {
        return new ExerciceRule($"Numbers should be different", (key) => GetNumbersInQuestion(key).Distinct().Count() == GetNumbersInQuestion(key).Count());
    }

    public static ExerciceRule GetRuleForNoSimpleNumberAbove(int max)
    {
        return new ExerciceRule($"No simple number above {max}", (key) => GetNumbersInQuestion(key).All(n => n % 5 != 0 || n <= max));
    }
}
